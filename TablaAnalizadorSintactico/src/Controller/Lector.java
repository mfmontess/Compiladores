...
/**
 * Clase que permite Leer y enviar los caracteres leidos a la clase Algoritmo
 * 
 * 
 */
public class Lector {
	...
	public Lector(File nFile) throws IOException {
		fileEntrada = nFile;
		abrirArchivoEntrada();
		leerArchivo();
	}

	// Metodo quecarga en el buffer la informacion del archivo de texto de
	// entrada
	...
	public void leerArchivo() throws IOException {
		...
		// adiciona la tabla de simbolos y cambia su tamaño para almacenar los
		// id resultantes
		totalresult = new char[0];
		pasintac = new String[0];
		...
		try {
			while ((linea = bufferDeArchivoEntrada.readLine()) != null) {
				...
				for (int i = 0; i <= longitud; i++) {
					...
					// se crea un nuevo objeto de tipo Algoritmo y se pasan por
					// referencia el caracter y las cadenas donde se almacenara
					// el resultado temporal y final
					Algoritmo algoritmo = new Algoritmo();
					algoritmo.Algoritmo(leido, result, totalresult, estado,
							tabla, contador, pasintac);
		...
		// se envia la cadena resultante y la tabla a las variables
		// correspondientes para que sean llamadas por la clase vista
		setTotalok(texto);
		setTabla(tabla);
		setPasintac(pasintac);

	}

	public String[] getPasintac() {
		return pasintac;
	}

	public void setPasintac(String[] pasintac) {
		this.pasintac = pasintac;
	}
